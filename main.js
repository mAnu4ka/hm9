...
const sord = (a) => {
    let butonse = document.querySelectorAll('.btn')
    ...
    let el = ''
    for (const key of butonse) {
        if (key.innerText == 'Добавлено') {
            key.push(a)
        } else if (a == 'arr_products') {
            CreatElement(a)
            return
        } else if (a == 'star') {
            key.push(a)
...
const CreatElement = (a) => {
    ...
    for (const item of a) {
        greed.innerHTML += ` 
            ...
            <button class='btn' id='${item.id}'>В избранное</button>
            </div>
            </div>
            `
    }
